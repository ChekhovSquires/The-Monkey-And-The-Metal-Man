import random
from games.utils.Logger import Logger


class HBDagent:
    def __init__(self, *args, print_logs=True):
        print(args)
        self.logger = Logger(print_logs=print_logs)

    def step(self, state):
        vals = [[0, 1], [0, -1], [1, 0], [-1, 0]]
        spotter_moves = [random.choice(vals), random.choice(vals)]
        spotter_direction = random.choice(vals)
        assasin_moves = [random.choice(vals), random.choice(
            vals), random.choice(vals), random.choice(vals)]
